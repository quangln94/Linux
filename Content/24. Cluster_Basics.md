## 1. MÔ HÌNH CHUNG CỦA HỆ THỐNG SERVER CLUSTER</br>
...
4. CÁC THÀNH PHẦN CỦA CLUSTER SERVICE
...
5. NGUYÊN TẮC HOẠT ĐỘNG CỦA SERVER CLUSTER
...
Heartbeat:
Là một UDP packet chuyển đổi giữa các node mỗi 1.2 giây một lần để xác định mỗi node trong Cluster vẫn hoạt động. Nếu một node thiếu hụt liên tiếp 5 heartbeat, node đó sẽ chuẩn bị một quá trình regroup event để đảm bảo rằng tất cả các node đi tới một sự nhất quán danh sách các node còn đang hoạt động.
...
Node failure và mất sự giao tiếp giữa các node:
